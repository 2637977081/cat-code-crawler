...
import com.xuxueli.crawler.GXxlCrawler;
...
import com.xuxueli.crawler.parser.strategy.GFilePageParser;
 ...
 *
 */
public class GCrawlerThread implements Runnable {
    private static Logger logger = LoggerFactory.getLogger(GCrawlerThread.class);

    private GXxlCrawler crawler;
    private boolean running;
    private boolean toStop;
    public GCrawlerThread(GXxlCrawler crawler) {
    ...
    public void run() {
        ...
        while (!toStop) {
            try {
                ...
                running = false;
                crawler.tryFinish();
                CrawlerTask task = crawler.getRunData().getTask();
                String turl = (task == null ? null : task.getUrl());
                running = true;
                logger.info(">>>>>>>>>>> xxl crawler, process link : {}", turl);
                if (!UrlUtil.isUrl(turl)) {
                    continue;
                }
                ...
                for (int i = 0; i < (1 + crawler.getRunConf().getFailRetryCount()); i++) {
                    ...
                    try {
                        // make request
                        PageRequest pageRequest = makePageRequest(turl);

                        // pre parse
                        crawler.getRunConf().getPageParser().preParse(pageRequest, task.getExtemplate());

                        // parse
                        if (crawler.getRunConf().getPageParser() instanceof GFilePageParser) {
                            ret = processFilePage(pageRequest, task.getExtemplate());
                        } else if (crawler.getRunConf().getPageParser() instanceof GPageParser){
                            ret = processGPage(pageRequest, task.getExtemplate());
                        } else if (crawler.getRunConf().getPageParser() instanceof NonPageParser){
     ...
     * @return boolean
     */
    private boolean processFilePage(PageRequest pageRequest, String extemplate){
        GFilePageParser filePageParser = (GFilePageParser) crawler.getRunConf().getPageParser();
        filePageParser.parse(pageRequest.getUrl(), extemplate);
        return true;
    }
     ...
     * @return boolean
     */
    private boolean processGPage(PageRequest pageRequest, String extemplate) {
        GPageParser gPageParser = (GPageParser) crawler.getRunConf().getPageParser();
        ...
        gPageParser.parse(pageRequest.getUrl(), pagesource, extemplate);
        return true;
    }
            ...
            if (links != null && links.size() > 0) {
                for (String item : links) {
                    if (crawler.getRunConf().validWhiteUrl(item)) {      // limit unvalid-child spread
                        crawler.getRunData().addTask(new CrawlerTask(item, ""));
                    }
                }
            }
        }
